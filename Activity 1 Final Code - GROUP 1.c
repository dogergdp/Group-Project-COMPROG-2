...
// function that prints a line of specified character and length
...
// function to act as placeholder for welcome banner
void banner(){
...
int main() {
    ...
    int arr_scores[NUM_OF_STUDENTS][NUM_OF_QUIZZES] = {}; // to store each student's score per quiz
    char id[NUM_OF_STUDENTS][20] = {}; // to store the ID number of each student.
    
    int i, j; // index
    float ave[NUM_OF_STUDENTS] = {}; // array to store average of each student
    int highest[NUM_OF_STUDENTS] = {}; // array to store the highest score of each student
    int lowest[NUM_OF_STUDENTS] = {}; // array to store the lowest score of each student
    int score; // used for calculations
...
banner();
  ...
  for (i=0; i<NUM_OF_STUDENTS; i++) {
    ...
    for (j=0; j<NUM_OF_QUIZZES; j++) {
      ...
      // checks and stores for the highest score of the current student
      if (j == 0 || highest[i] < score) {
        highest[i] = score;
      }

      // check and stores for the lowest score of the current student
